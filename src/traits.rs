use { encode, encode_with_alphabet, decode, decode_with_alphabet, DecodeError };
...
impl FromBase58 for str {
    #[allow(deprecated)]
    fn from_base58(&self) -> Result<Vec<u8>, FromBase58Error> {
        decode(self).map_err(FromBase58Error::from)
    }

    #[allow(deprecated)]
    fn from_base58_with_alphabet(&self, alpha: &[u8; 58]) -> Result<Vec<u8>, FromBase58Error> {
        decode_with_alphabet(self, alpha).map_err(FromBase58Error::from)
    }
}
